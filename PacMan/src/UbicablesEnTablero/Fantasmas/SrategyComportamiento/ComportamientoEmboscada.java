...
public class ComportamientoEmboscada implements InterfazComportamiento {
    private final int DISTANCIA_EMBOSCADA = 5;
    ...
    public int[] direccionar(DuplaDoble miUbicacion, DuplaDoble pacUbicacion, int pacDireccion) {
        ...
        switch(pacDireccion){
            case 0:{
                dx = miUbicacion.getX()-(pacUbicacion.getX()-DISTANCIA_EMBOSCADA);
                dy = miUbicacion.getY()-pacUbicacion.getY();
            }
            break;
            case 1:{
                dx = miUbicacion.getX()-pacUbicacion.getX();
                dy = miUbicacion.getY()-(pacUbicacion.getY()-DISTANCIA_EMBOSCADA);
            }
            break;
            case 2:{
                dx = miUbicacion.getX()-(pacUbicacion.getX()+DISTANCIA_EMBOSCADA);
